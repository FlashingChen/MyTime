...
- 点击开始/结束计时；运行中的会话和待确认会话会持久化，进程被杀后可恢复。系统时钟回拨不超过 5 分钟时继续计时（已计时长按 0 显示），回拨过大或在停止时发现负/零时长（无法保存为有效记录）则重置会话并提示。
...
- 记录、分类和普通设置默认仅存储在本机应用目录；离开设备的数据去向见本条与下文（iCloud 备份、WebDAV、剪贴板导入导出）。
- 数据备份：Android 已禁用 Google 云备份（`android:allowBackup="false"`，API 31+ 另经 `dataExtractionRules` 排除），记录、分类、同步快照与设置不会上传 Google 备份；部分厂商设备上系统设备间传输（D2D）仍可能迁移应用数据。iOS 数据（Hive 数据库与偏好设置，含同步快照）随 iCloud 备份。换机或恢复出厂后可通过 JSON 导出/导入或 WebDAV 同步恢复。
- 凭据（AI API Key / WebDAV 密码）位于平台安全存储：Android Keystore 主密钥不随任何备份或迁移，恢复或换机后需重新输入；iOS 钥匙串项不随 iCloud 备份，但从加密的电脑备份恢复或设备迁移时可能随迁，建议恢复后核对凭据。
...
- [统计与时间线视觉优化设计](docs/superpowers/specs/2026-07-21-stats-timeline-polish-design.md)
- [WebDAV 后台记录合并设计](docs/superpowers/specs/2026-07-22-webdav-background-record-merge-design.md)
- [前台持有 WebDAV 同步设计](docs/superpowers/specs/2026-07-24-foreground-owned-webdav-sync-design.md)
