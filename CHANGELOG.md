...
- 网络响应体读取增加超时：WebDAV（2 分钟）与 AI 请求（30 秒）在响应头到达后仍慢速或永不完成 body 时不再永久挂起 UI；WebDAV 同步面板对超时显示独立提示，AI 请求沿用"请求超时"提示。
- Android 禁用系统自动备份与设备间传输（`android:allowBackup="false"`）；iOS 明确数据随 iCloud 备份，恢复后需重新输入安全存储凭据。README 的"隐私与安全"补充纯本地定位下的数据去向。
- 计时器对系统时钟回拨增加防护：回拨 5 分钟内将已计时长钳制为零并继续计时，超过阈值视为会话异常，重置计时并通过提示告知；停止/恢复路径同样钳制负时长，避免界面显示 `-1:59:59` 这类负值。
- 代码健壮性与隐私细节（Issue #23）：`TimeRecord` 相等性纳入 `createdAt`，JSON 备份导出/导入完整往返 `createdAt`（旧备份缺失时回退到开始时间）；数据导出改为生成 JSON 文件经系统分享面板保存，不再把含备注的明文全量数据写入剪贴板；AI 服务地址校验前后一致——非本机地址仅接受 HTTPS，`http://localhost` 等回环地址显式放行并在表单中给出明确提示；WebDAV 同步进行中若服务器配置变更，不再复用旧配置的 in-flight 请求（指纹含 endpoint/用户名/密码）；RecordsBloc 移除 `_ignoreNextRepositoryChange` 时序标志，仓库外部变更不再可能被吞掉；iOS 后台同步补齐 `BGTaskSchedulerPermittedIdentifiers…
- 时间线布局（Issue #21 第 1 项）：位置计算改为自当日零点起的绝对秒差归一化——结束于午夜整点的记录不再被压成最小卡片高度（23:00–00:00 正确显示 60px），跨午夜记录裁剪后延伸至当日结束（22:30–次日 00:00 显示 90px），含秒的开始时间保持精确位置（10:30:45 → 630.75px）。
...
### Added
- 计时提醒：计时开始后每隔可自定义的间隔（10–120 分钟预设）通过系统通知提醒用户停止计时，避免忘记停止；间隔以计时起点为基准递增，应用被杀后由系统继续展示已排期的提醒（重启后自动重排），停止或重置计时即取消。设置页可开关并选择间隔，首次开启时请求通知权限。
